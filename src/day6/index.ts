...
type Order = [Command, Coordinate, Coordinate]
type LightArray = Array<Array<number>>

export class Day6 {
    private static buildLightArray(size: number): LightArray {
        let lightArray = []
        for (let i = 0; i < size; i++) {
            lightArray.push(Array(size).fill(0))
        }
        return lightArray;
    }

    private static parseInput(input: String): Array<Order> {
        ...
        input.split("\n").map(line => {
            ...
            from = fromAndTo[0].split(",").map(x => Number(x)) as Coordinate
            if (typeof fromAndTo[1] === 'undefined')
                console.log("yo", line)
            to = fromAndTo[1].split(",").map(x => Number(x)) as Coordinate
            orders.push([command, from, to])
        })

        return orders;
    }

    static runOrders(orders: Array<Order>, lightArray: LightArray): LightArray {
        orders.forEach(order => {
            for (let x = order[1][0]; x <= order[2][0]; x++) {
                for (let y = order[1][1]; y <= order[2][1]; y++) {
                    switch (order[0]) {
                        case "+":
                            lightArray[x][y] = 1;
                            break;
                        case "-":
                            ...
                            break;
                        case "*":
                            lightArray[x][y] = lightArray[x][y] === 1 ? 0 : 1;
                            break;
                    }
                }
            }
    ...
    private static calculateLights(lightArray: LightArray): number {
        let cnt = 0;
        for (let i = 0; i < lightArray.length; i++) {
            for (let j = 0; j < lightArray[i].length; j++) {
                cnt += lightArray[i][j] ? 1 : 0
            }
        }
        return cnt
    }

    private static prettyPrint(lightArray: LightArray): void {
        for (let i = 0; i < lightArray.length; i++) {
            console.log(lightArray[i].map(x => x ? "+" : "-"))
        }
    }

    static solve1() {
        console.log(this.calculateLights(this.runOrders(this.parseInput(this.input), this.buildLightArray(1000))))
    }

    static runCorrectOrders(orders: Array<Order>, lightArray: LightArray): LightArray {
        orders.forEach(order => {
            for (let x = order[1][0]; x <= order[2][0]; x++) {
                for (let y = order[1][1]; y <= order[2][1]; y++) {
                    switch (order[0]) {
                        case "+":
                            ...
                            break;
                        case "-":
                            lightArray[x][y] += lightArray[x][y] - 1 < 0 ? 0 : -1;
                            break;
                        case "*":
                            lightArray[x][y] += 2;
                            break;
    ...
    private static calculateCorrectedLights(lightArray: LightArray): number {
        let cnt = 0;
        for (let i = 0; i < lightArray.length; i++) {
            for (let j = 0; j < lightArray[i].length; j++) {
                cnt += lightArray[i][j]
    ...
    static solve2() {
        console.log(this.calculateCorrectedLights(this.runCorrectOrders(this.parseInput(this.input), this.buildLightArray(1000))))
    }

    static input3 = `turn off 0,0 through 2,2`
    static input2 = `turn on 887,9 through 959,629
...
toggle 831,394 through 904,860`
    static input = `turn on 887,9 through 959,629
turn on 454,398 through 844,448
turn off 539,243 through 559,965
turn off 370,819 through 676,868
turn off 145,40 through 370,997
